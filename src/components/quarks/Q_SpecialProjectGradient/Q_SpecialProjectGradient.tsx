...
function shouldRunPixiGradient() {
  return (
    !prefersReducedMotion() && !hasConstrainedConnection() && !hasWeakHardware() && canUseWebGL()
  );
}
...
const Q_SpecialProjectGradient = () => {
  ...
  useEffect(() => {
    ...
    if (!shouldRunPixiGradient()) {
      return undefined;
    }

    const initPixi = async () => {
      const [PIXI, { DisplacementFilter }] = await Promise.all([
        import('pixi.js'),
        import('@pixi/filter-displacement')
      ]);

      if (isDisposed || !root.isConnected) {
        return;
      }
      ...
      let gradientSprite: InstanceType<typeof PIXI.Sprite> | null = null;
      let displacementSprite: InstanceType<typeof PIXI.Sprite> | null = null;
      let displacementFilter: InstanceType<typeof DisplacementFilter> | null = null;
      let maskSprite: InstanceType<typeof PIXI.Sprite> | null = null;
      let overlaySprite: InstanceType<typeof PIXI.Sprite> | null = null;
        ...
        for (let y = 0; y < height; y += 1) {
          for (let x = 0; x < width; x += 1) {
            ...
            const value2 = Math.cos(
              (normalizedY * frequency2 - normalizedX * 0.6 * frequency2) * Math.PI * 2 -
                phase * 0.7
            );
